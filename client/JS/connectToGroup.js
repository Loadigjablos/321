function connectToGroup() {
  try {
    let allWindows = document.getElementsByClassName(
      "absolute top-[25%] left-[35%] text-black w-[35%] text-[1rem] flex flex-col bg-white justify-center"
    );
  ...
  connectDiv.className =
    "relative flex mb-[1.5rem] h-[3.375rem] cursor-pointer";
  roomName.className =
    "w-[80%] h-[100%] text-[1.2rem] bg-[rgba(0,0,0,0)] mt-[1rem] rounded-[2px] border-2 m-auto placeholder:text-[1rem]";
  connectLabel.className =
    "absolute left-[3rem] top-0 text-[1.4rem] font-normal text-[rgb(112,117,121)] px-[0.25rem] bg-white";
  connectToChat.className =
    "w-auto border-black m-auto text-[1.4rem] rounded-[0.75rem] bg-[rgb(218,33,110)] p-2 mb-2";
  closeWindow.className =
    "absolute right-2 top-0 text-[2rem] text-red-600 hover:bg-gray-300 z-10";
  //Events
  ...
  connectToChat.addEventListener("click", function () {
    ...
    request.send(JSON.stringify({ name: roomName.value }));
